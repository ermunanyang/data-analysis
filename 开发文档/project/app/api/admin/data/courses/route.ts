...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (teacher) {
      where.OR = [{ teacherNames: { contains: teacher } }, { ownerTeacher: { contains: teacher } }];
    }
    ...
    const [courses, total] = await Promise.all([
      prisma.course.findMany({
        ...
        include: {
          ...
          _count: { select: { students: true, targets: true, methods: true } },
        },
    ...
    return NextResponse.json({
      success: true,
      data: { courses: formattedCourses, pagination: { page, pageSize, total, totalPages: Math.ceil(total / pageSize) } },
    });
  } catch (error) {
