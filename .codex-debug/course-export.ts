...
import { calculateCourse } from "./course-calculator";
import type { CourseInput } from "./course-schema";
...
function buildStudentTargetAttainmentSheet(
  ...
  const sheet = workbook.addWorksheet("4学生课程目标达成�?);
  const { processMethods, resultMethod } = getMethodSets(course);
...
async function loadStandaloneChartTemplate() {
  ...
  throw new Error("未找到包含�?绘图数据”工作表�?Excel 模板文件");
}
...
function fillStandaloneChartSheet(
  ...
  sheet.getCell("C1").value = "   平均达\n     成度\n姓名";
  sheet.getCell("K1").value = "总目�?;
  sheet.getCell("T1").value = "总目�?;
  sheet.getCell("U1").value = "期望�?;
  sheet.getCell("M2").value = "平均达成�?;
...
function applyDefaultSheetStyling(workbook: ExcelJS.Workbook) {
  workbook.worksheets.forEach((sheet) => {
    if (sheet.name === "4学生课程目标达成�? || sheet.name === "5绘图数据") {
      return;
    }
...
async function loadReportTemplateZip() {
  const templatePath = path.join(process.cwd(), "3课程达成度分析报告模�?xlsx");
...
function buildTemplateChartSheetXml(
  ...
  entries.push(["K1", "总目�?], ["T1", "总目�?], ["M2", "平均达成�?], ["U1", "期望�?], ["K2", calc.averages.totalAverage]);
...
function buildTemplateReportSheetXml(
  ...
  Array.from({ length: 7 }, (_, targetIndex) => {
    ...
    entries.push([`A${rowIndex}`, target ? "课程总目�? : null]);
  ...
  Array.from({ length: 7 }, (_, targetIndex) => {
    ...
    entries.push([`A${oddRow}`, target?.name ?? null], [`B${oddRow}`, target?.name ?? null], [`C${oddRow}`, target ? "小题�? : null]);
    entries.push([`A${evenRow}`, target?.name ?? null], [`B${evenRow}`, target?.name ?? null], [`C${evenRow}`, target ? "分�? : null]);
  ...
  Array.from({ length: 7 }, (_, targetIndex) => {
    ...
    entries.push([`A${baseRow}`, target?.name ?? null], [`B${baseRow}`, "直接评价"], [`C${baseRow}`, "过程性评�?], [`R${baseRow}`, summary?.overallWeight ?? 0], [`T${baseRow}`, summary?.finalAttainment ?? 0]);
    ...
    entries.push([`A${resultRow}`, target?.name ?? null], [`C${resultRow}`, "结果性评�?], [`I${resultRow}`, summary?.resultWeight ?? 0], [`K${resultRow}`, summary?.resultTargetScore ?? 0], [`N${resultRow}`, summary?.resultAverageScore ?? 0], [`P${resultRow}`, summary?.resultAttainment ?? 0]);
