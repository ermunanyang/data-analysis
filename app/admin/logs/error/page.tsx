...
export default function ErrorLogsPage() {
  ...
  const handleResolve = async (logId: string) => {
    try {
      const res = await fetch("/api/admin/logs/error", {
        method: "PUT",
  ...
  const columns: TableColumnsType<ErrorLog> = [
    ...
    },
    {
      ...
      dataIndex: "severity",
      key: "severity",
      render: (sev: string) => <Tag color={getSeverityColor(sev)}>{sev}</Tag>,
    },
    {
      title: "状态",
      dataIndex: "resolved",
        ...
        {selectedLog && (
          <Descriptions bordered column={2}>
            ...
            <Descriptions.Item label="严重程度">
              <Tag color={getSeverityColor(selectedLog.severity)}>{selectedLog.severity}</Tag>
            </Descriptions.Item>
            ...
            <Descriptions.Item label="解决时间">
              {selectedLog.resolvedAt ? dayjs(selectedLog.resolvedAt).format("YYYY-MM-DD HH:mm") : "-"}
            </Descriptions.Item>
