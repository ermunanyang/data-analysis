  ...
  const errorColumns: TableColumnsType<DashboardStats["recentErrors"][0]> = [
    ...
    { title: "错误信息", dataIndex: "errorMessage", key: "errorMessage", ellipsis: true },
    {
      ...
      render: (severity: string) => (
        <Tag color={severity === "critical" ? "red" : severity === "error" ? "orange" : "yellow"}>
          {severity}
        </Tag>
      ),
    },
      ...
      <Row gutter={[16, 16]} style={{ marginTop: 24 }}>
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic
              title="操作日志"
              value={stats?.operationLogCount || 0}
      ...
      <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic
              title="错误日志"
              value={stats?.errorLogCount || 0}
              prefix={<WarningOutlined style={{ color: "#f5222d" }} />}
              valueStyle={{ color: stats?.errorLogCount && stats.errorLogCount > 0 ? "#f5222d" : "#52c41a" }}
            />
          </Card>
        </Col>
      </Row>
      <Card style={{ marginTop: 24 }} title="最近错误">
        <Table
